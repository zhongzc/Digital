...
import de.neemann.digiblock.gui.remote.DigiblockHandler;
...
/**
 * The main frame of the Digiblock Simulator
 * Set log level: -Dorg.slf4j.simpleLogger.defaultLogLevel=debug
 */
public final class Main extends JFrame implements ClosingWindowListener.ConfirmSave, ErrorStopper, FileHistory.OpenInterface, DigiblockRemoteInterface, StatusInterface, ChangedListener {
    ...
    private void setFilename(File filename, boolean toPrefs) {
        ...
        if (filename != null) {
            ...
            setTitle(prefix + filename + " - " + Lang.get("digiblock"));
        } else {
            setTitle(prefix + Lang.get("digiblock"));
        }
    }
    ...
    public static void main(String[] args) {
        Thread.setDefaultUncaughtExceptionHandler(new DigiblockUncaughtExceptionHandler());
        ...
        if (file != null && file.getName().endsWith(".fsm")) {
            ...
            SwingUtilities.invokeLater(() -> {
                ...
                try {
                    new RemoteSever(new DigiblockHandler(main)).start(41114);
                } catch (IOException e) {
